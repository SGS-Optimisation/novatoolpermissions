    ...
    <client-layout :client-account="clientAccount">
      ...
      <template #body>
        <div class="max-w-7xl mx-auto sm:px-4 lg:px-2 bg-gray-50 pt-5">
          <div class="search-section mt-2 grid grid-cols-12 gap-1">
            ...
            <div class="col-span-3">
              <taxonomy-selector taxonomy-name="Contributing Team"
                                 :terms="allTeams"
          ...
          <div class="search-section grid grid-cols-5 gap-1">
            <template v-for="(terms, taxonomyName) in termsByTaxonomies">
              <taxonomy-selector v-if="groupedTaxonomies['Job Categorizations'].includes(taxonomyName)"
          ...
          <div class="search-wrapper transition transition-all duration-250 ease-out"
            ...
            <div :class="{'hidden': !showAdvancedSearch}">
              <div class="search-section grid grid-cols-5 gap-1">
                <template v-for="(terms, taxonomyName) in termsByTaxonomies">
                  <taxonomy-selector v-if="groupedTaxonomies['Account Structure'].includes(taxonomyName)"
                                     :taxonomy-name="taxonomyName"
                                     :key="taxonomyName"
                                     :terms="terms"
                                     @termSelected="filterByTaxonomyTerm"
          ...
          <div class="flex justify-between">
            <div class="flex">
              ...
              <!-- sorting -->
              <div class="flex m-2">
                <jet-label class="align-middle mr-2 mt-2" value="Sorting:"/>
          ...
          <div class="flex flex-row w-full justify-between">
            <div class="flex">
              ...
              Select:
              <a class="border border-blue-600 rounded text-blue-500 cursor-pointer px-1 mx-1 hover:bg-blue-200"
                 @click="selectAll">
                All
              </a>
              <a class="border border-blue-600 rounded text-blue-500 cursor-pointer px-1 mx-1 hover:bg-blue-200"
    ...
    initializeFilters() {
      ...
      this.filterObject['filterRuleType'] = (itemElem) => {
        return !this.filterRuleType
            || (Array.isArray(this.filterRuleType) ?
                _.every(this.filterRuleType, (type) => itemElem[type])
                : itemElem[this.filterRuleType]);
      };
