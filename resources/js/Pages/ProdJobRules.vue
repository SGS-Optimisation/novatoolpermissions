<template>
  <app-layout>
    <Head><title>
      Prod Rules {{currentJob.job_number}} - Dagobah
    </title></Head>
    <template #header>
...
export default {
  ...
  computed: {
    ...
    numDisplayedRules() {
      ...
      for (let stage in this.stageStates) {
        if (this.stageStates[stage]) {
          stages.push(stage);
        }
      }
            ...
            if(stages.length) {
              rules = rules.filter(rule => {
                return (
                    ...
                    || _.some(
                        rule.job_categorizations_terms,
                        (term) => term.taxonomy.name === 'Stage' && stages.includes(term.name)
                    )
                );
              });
            }
