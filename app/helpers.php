...
function getTaskStatusDescription($taskId)
{
    $match = Arr::where(config('mysgs.task_status'), function ($value, $key) use ($taskId) {
        return $value['taskStatusId'] == $taskId;
    });

    if ($match) {
        return $match['taskStatusDescription'];
    }
